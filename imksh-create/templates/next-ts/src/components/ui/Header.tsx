...
const Header = () => {
  ...
  return (
    <header ref={headerRef as any} className="sticky top-0 z-40 p-4">
        ...
        <div className="bg-primary text-primary-content rounded-full shadow-lg px-5 py-2.5 flex items-center justify-between relative">
          ...
          <div className="flex items-center gap-3">
            <Link href="/" className="flex items-center gap-2">
              {appConfig.app.logo ? (
                <img src={appConfig.app.logo} alt="logo" className="h-10 w-10" />
              ) : (
            ...
            {navigation.map((item) => (
              <Link 
                ...
                className={`hover:text-primary transition-colors ${
                  pathname === item.path ? "text-white font-bold" : "text-white/80"
                }`}
              >
                {item.title}
              </Link>
                ...
                {navigation.map((item) => (
                  <Link
                    ...
                    className={`block px-4 py-3 rounded-xl hover:bg-white/10 transition-colors font-semibold ${
                      pathname === item.path ? "bg-white/15 text-white" : "text-white/80"
                    }`}
                  >
                    {item.title}
                  </Link>
